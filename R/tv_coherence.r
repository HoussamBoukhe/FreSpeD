...
#' @description Compute the time varying cross-coherence of two signals
#' 
...
tv_coherence<-function (X, windowLen, overlap = 0, M_welch = windowLen/2 * 0.1, 
                        logScale = FALSE, padEnd = TRUE, transform = "", plot = FALSE,bands_analysis = FALSE, f_sampling = NULL, filter_tmp = FALSE, fmin, fmax) 
{
  if (is.null(ncol(X))) 
  ...
  for (iInterval in 1:ncol(Y1)) {
    ...
    for (iFB in 1:M_welch) {
      ...
      if (filter_tmp) {
        Y1int <- preprocess_signals(Y1int, f_sampling = f_sampling, fmin = fmin, fmax = fmax)$V1
      }

      Y2int <- Y2[((iFB * fWindowLen) - fWindowLen + 1):(iFB * fWindowLen), iInterval] 
      Y2int <- Y2int - mean(Y2int)
      if (filter_tmp) {
        Y2int <- preprocess_signals(Y2int, f_sampling = f_sampling, fmin = fmin, fmax = fmax)$V1
      }
  ...
  if (bands_analysis){
    ...
    bands_names <- expression(delta, theta, alpha, beta, gamma)
    # Select corresponding labels for breaks
    selected_breaks <- seq(dim(coh)[2])[selected_indices]
    
    # Plot heatmap with custom x-axis and y-axis labels
    p <- ggplot(df, aes(x = Var2, y = factor(Var1, levels = seq(dim(coh)[1]), labels = if (bands_analysis) bands_names else custom_labels_y), fill = value)) +
      geom_tile() +
      scale_fill_gradient(low = "white", high = "black") +  # Define colors
      labs(x = "time", y = if (bands_analysis)"Frequency bands" else "Fundamental frequencies", title = "Cross-coherence") +  # Add labels
