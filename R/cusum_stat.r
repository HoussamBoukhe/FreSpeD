...
#' @description This function applies the UHcusum function to the estimated spectral quantities. If the cusum value is bigger than
...
cusum_stat <- function(S, thresh = 0.8 * log(dim(S)[2])^(1.1), check_neighborhood = TRUE, 
  ...
  if (n > (2 * delta + 1)) {
    ...
    # Check for spurious change points in the neighborhood, beacuse
