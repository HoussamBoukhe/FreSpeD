                       ...
                       normalize = FALSE, padEnd = TRUE, logScale = FALSE, bands_analysis = FALSE, f_sampling = NULL,plot = FALSE, ...) 
{
  ...
  # ((windowLen/2/M_welch) < 4 || (windowLen/2/M_welch) > (windowLen/2))   
  ...
  for (iInterval in 1:ncol(Y)) {
    ...
    if (bands_analysis){
      ...
      indices <- get_bands_indices(2*nrow(S), f_sampling)
  ...
  if (plot) {
    ...
    p <- ggplot(df, aes(x = Var2, y = factor(Var1, levels = seq(dim(S)[1]), labels = if (bands_analysis) bands_names else custom_labels_y ), fill = value)) +
      geom_tile() +
      scale_fill_gradient(low = "white", high = "black") +  # Define colors
      labs(x = "time", y = if (bands_analysis)"Frequency bands" else "Fundamental frequencies", title = "Periodogram") +  # Add labels
